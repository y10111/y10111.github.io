课程链接：视频《How the C++ Compiler Works》（The Cherno C++ 系列 第 6 集）
...
.obj 是二进制，无法直接阅读。转换成人能读的形式：项目属性 → C/C++ → 输出文件 → 把"汇编程序输出"设为**"仅程序集列表"**，编译后在目录里得到 **math.asm** 文件——它就是 .obj 内容的可读版本，包含机器码对应的汇编指令。
...
**函数调用与签名**：让 Multiply 里调用另一个函数 Log（为简化，Log 不打印、只是原样返回收到的指针参数）。此时汇编里在乘法之前出现一条 **call 指令**调 Log。注意 Log 在汇编里被一堆 "@" 和随机字符"装饰"——那是**函数签名**，用来唯一标识函数，链接器拼接 .obj 时靠它查找匹配（细节见链接专题）。
