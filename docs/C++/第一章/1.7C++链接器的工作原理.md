...
修改 log.cpp 里的 Log，把返回类型从 void 改成 int（内部返回 0）→ 生成报链接错误。因为 math.cpp 里声明的是"返回 void 的 log"。链接器寻找的是**特定签名**的函数：返回类型 + 函数名 + 参数列表（含调用约定）必须全对。

再给它加第二个参数 level → 同样报链接错误，信息会精确描述它期待的函数：返回 void、名为 log、只有一个 const char* 参数。恢复原状，程序又能链接通过。

```mermaid
flowchart LR
    A["调用方声明<br/>void Log(const char*)"] --> B["链接器按完整签名查找"]
    B --> C{找到匹配定义？}
    C -->|是| D["链接成功"]
...
**方式二：加 inline**——inline（内联）的含义是：用函数体直接替换调用处（调用 Log 的地方变成那段函数内容），不再生成一个独立的对外链接函数，从而避免冲突。

**方式三（推荐）**：**把定义移进唯一一个翻译单元，头文件只留声明**。因为错误的本质是同一个定义被 contain 进两个翻译单元。既然它与日志相关，就把定义放进 log.cpp，log.h 里只保留形如 `void Log(const char* message);` 的声明，其他文件包含头文件仅获得声明，定义全局唯一：
